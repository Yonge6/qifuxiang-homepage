...
export default function Home({ runtime = false }: { runtime?: boolean }) {
  ...
  const [certFamily, setCertFamily] = useState('体系认证');
  ...
  const toolboxContent = <div className="toolbox-page">
    <section className="toolbox-hero" aria-labelledby="toolbox-title">
      <div className="toolbox-hero-copy"><span>QIFUXIANG · BUSINESS TOOLKIT</span><h1 id="toolbox-title">企业服务工具箱</h1><p>查询、办理、评测，一站快速完成</p><div className="toolbox-hero-meta"><b>企业查询</b><b>业务办理</b><b>智能评测</b></div></div>
  ...
  const certificationServices = [
    { name: '厂房消防一证', tags: ['消防验收', '一证办理'], price: '0.01', sold: '已售666888', image: 'cert-fire.webp' },
    { name: '道路运输经营许可证', tags: ['道路运输', '经营许可'], price: '0.10', sold: '已售666888', image: 'cert-transport.webp' },
    { name: '企业官网搭建', tags: ['官网建设', '企业展示'], price: '0.10', sold: '已售666888', image: 'cert-website.webp' },
    { name: 'CMMI 基础认证', tags: ['体系认证', '能力成熟度'], price: '0.10', sold: '已售666888', image: 'cert-cmmi.webp' },
    { name: '应收账款催收', tags: ['应收账款', '法律服务'], price: '0.10', sold: '已售666888', image: 'cert-debt.webp' },
  ];
  const certificationContent = <div className="certification-page">
    <section className="certification-hero" aria-labelledby="certification-title"><div className="certification-copy"><span>企服象 · 认证服务中心</span><h1 id="certification-title">体系认证，一站办妥</h1><p>覆盖资质、体系与科技项目申报服务</p></div><div className="certification-mark" aria-hidden="true"><ShieldCheck size={55} weight="duotone" /><i><CheckCircle size={19} weight="fill" /></i></div></section>
    <div className="certification-sections"><section className={`certification-nav ${certExpanded ? 'expanded' : ''}`} aria-label="分类筛选"><div className="certification-nav-grid">{categories.map(c => <button className={certFamily === c.name ? 'active' : ''} key={c.name} aria-pressed={certFamily === c.name} onClick={() => { setCertFamily(c.name); goTop(); }}><span><img src={img(c.icon === 'shield' ? 'icon-soft-shield-flat.png' : `icon-soft-${glassIcons[c.icon]}.webp`)} alt="" /></span><strong>{c.name}</strong></button>)}<button className={`certification-all ${certFamily === '全部' ? 'active' : ''}`} aria-pressed={certFamily === '全部'} onClick={() => { setCertFamily('全部'); goTop(); }}><span><SquaresFour size={24} /></span><strong>全部</strong></button><button className="certification-expand" aria-expanded={certExpanded} onClick={() => setCertExpanded(value => !value)}><span><CaretDown size={18} /></span><strong>{certExpanded ? '收起' : '展开'}</strong></button></div></section>
      {certFamily === '体系认证' ? <section className="certification-catalog"><div className="certification-tabs" role="tablist" aria-label="体系认证子类">{['体系认证1','体系认证2','体系认证3','体系认证4'].map(tab => <button role="tab" aria-selected={certTab === tab} className={certTab === tab ? 'active' : ''} key={tab} onClick={() => setCertTab(tab)}>{tab}</button>)}</div><div className="certification-summary"><strong>共 49 项服务</strong></div><div className="certification-list">{certificationServices.map(service => <button className="product-card certification-service" key={service.name} onClick={() => consultTool(service.name)}><img src={img(service.image)} alt={service.name} loading="lazy" width="320" height="320" /><div className="product-info"><h3>{service.name}</h3><div className="product-tags" aria-label="服务标签">{service.tags.map(tag => <span key={tag}>{tag}</span>)}</div><div className="product-bottom"><span className="price"><small>¥</small>{service.price}<small className="price-suffix">起</small></span><span className="social-proof">{service.sold}</span></div></div></button>)}</div></section> : <section className="certification-empty-page" aria-label={`${certFamily}空页面`} />}</div>
    <footer className="page-footer"><div className="footer-brand"><ShieldCheck size={20} />企服象</div><p>专业服务 · 助力企业发展</p></footer>
  </div>;
  const content = <>
    ...
    <div className="page-sections">
      ...
      <Dashboard />
      <section className="panel category-panel" aria-label="企业服务分类"><div className="category-grid">{categories.map(c => { return <button key={c.name} onClick={() => { if (c.name === '体系认证') { setCertFamily('体系认证'); setHomeView('certification'); goTop(); } else openSearch(c.name); }}><span className="category-icon"><img src={img(c.icon === 'shield' ? 'icon-soft-shield-flat.png' : `icon-soft-${glassIcons[c.icon]}.webp`)} alt="" width="64" height="64" /></span><span>{c.name}</span></button>; })}</div></section>
  ...
  return <div className={`qfx-app ${runtime ? 'qfx-runtime' : 'qfx-h5'}`} ref={page}>
    {activeNav === 'home' && homeView === 'certification' ? <header className="topbar certification-topbar"><button className="certification-nav-back" aria-label="返回首页" onClick={() => { setHomeView('home'); goTop(); }}><ArrowLeft size={21} /><span>返回</span></button><strong>体系认证</strong><div className="wechat-capsule"><button aria-label="更多信息" onClick={() => show('menu')}><DotsThree size={24} weight="bold" /></button><button aria-label="回到页面顶部" onClick={goTop}><Record size={22} weight="bold" /></button></div></header> : <header className="topbar"><button className="location-button" onClick={() => show('location')}>{location}<CaretDown size={12} /></button><button className="search-trigger" onClick={() => openSearch()}><MagnifyingGlass size={20} /><span>搜索</span></button><div className="wechat-capsule"><button aria-label="更多信息" onClick={() => show('menu')}><DotsThree size={24} weight="bold" /></button><button aria-label="回到首页顶部" onClick={goTop}><Record size={22} weight="bold" /></button></div></header>}
