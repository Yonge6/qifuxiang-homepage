...
export default function Home({ runtime = false }: { runtime?: boolean }) {
  ...
  const content = <>
    ...
    <div className="page-sections">
      ...
      <section className="panel dashboard" aria-labelledby="dashboard-title"><div className="dashboard-heading"><h2 id="dashboard-title">数据看板</h2><p>最近一周的商标数据统计，仅供参考</p></div><div className="metrics">{[['申请量', '187', '件'], ['下证量', '64', '件'], ['驳回量', '48', '件']].map(([label, value, unit]) => <div className="metric" key={label}><span>{label}</span><div><strong>{value}</strong><small>{unit}</small></div></div>)}</div></section>
      <section className="panel category-panel" aria-label="企业服务分类"><div className="category-grid">{categories.map(c => { const Icon = iconMap[c.icon]; return <button key={c.name} onClick={() => openSearch(c.name)}><span className="category-icon"><Icon size={27} weight="duotone" /></span><span>{c.name}</span></button>; })}</div></section>
