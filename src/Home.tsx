...
export default function Home({ runtime = false }: { runtime?: boolean }) {
  ...
  const productCard = (p: Product) => <button className="product-card" key={p.id} onClick={() => openProduct(p)} aria-label={`查看${p.title}`}><img src={img(p.image)} alt={p.title} loading="lazy" width="320" height="320" /><div className="product-info"><h3>{p.title}</h3><p>{p.subtitle}</p><div className="product-bottom"><span className="price"><small>¥</small>{p.price}<small className="price-suffix">起</small></span><span className="social-proof">{p.proof}</span></div></div></button>;
  const content = <>
