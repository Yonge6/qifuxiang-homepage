...
- 用户选定蓝白建筑摄影版本，要求可通过 GitHub 访问的 H5 首页。
- 首页长页必须保留数据看板；分类入口按当前小程序实际配置（本次为 3 个，不虚构补足）；热门服务双列、标题最多两行；之后依次资讯公告、企业服务。
- 中央客服固定突出。仅首页展示交互，不扩展后台/支付/真实消息。
- 独立 H5 入口 h5.html 复用 Home；原模拟器入口与全部 protected runtime 保留不变。

- 2026-09-20 最新要求：全部首页业务内容从实际小程序采集；热门资讯和知识科普各四条，企业服务保留初创期/成长期/成熟期三个页签，每页三卡；仅优化 UI。动态数据为采集快照，不宣称后台实时同步。
- 品质精修方向：保持蓝色科技风、现有信息架构和真实内容；采用深蓝文字、冰蓝表面、精细边框与柔和阴影，统一图标色系，提升小字可读性。不要为美化改写商品或新闻。

- 最新用户覆盖：明确授权首页分类补成两行八个，第一行体系认证/工商代办/政策补贴/科技申报，第二行财税服务/知识产权/资质许可/法律服务；图标按附图彩色通透风格。这八项是设计定义，其他业务数据仍保留真实采集内容。

- 参考图风格再对齐：使用用户提供的玻璃飘带主视觉原图、前四个原始立体图标；新增四项使用同风格独立生成素材。保留八宫格和双列商品，不再用线性图标替代立体素材。

- 最新批注：商品/资讯/企业服务标题统一15px（窄屏不缩小）；辅助正文13px、元信息至少12px。看板标签/数字统一颜色，透明底，两道竖线；两页左右滑动、自动播放，仅分页点，无播放按钮。第二页按用户截图为申请量(新办|年审)284家 / 发证量3201张 / 失效(注销|撤销)980张。商品图片贴边；企业服务改横向卡片；全站服务图标统一半扁平，避免厚玻璃质感。删除底部来源日期行。

- 底部五个导航图标统一重绘，未选中灰蓝描边、选中蓝色填充，状态跟随当前打开的面板；客服保留中央突出位置，关闭面板后恢复首页选中。看板说明为明确例外：10px，位于标题右侧，整体紧凑高度。

- 最新覆盖：底部Tab直接切换空白占位页，不能弹窗遮住选中态；首页恢复真实首页。企业服务九项各用不同语义图标。通知条与Banner间距10px。看板说明字号随容器可用宽度放大，保持单行；分页点更紧凑。商品四张封面由Ego ChatGPT生成，各保留蓝认证/橙金融/红白人物/红黑审计电商风格。新增两张Banner，图片不带分页点，页面分页点居中。

- 企业服务九个图标最终统一为八宫格同风格的独立半扁平彩色插画素材，避免线框图标。Banner每5秒自动轮播，手动交互后暂停一轮。数据看板删除底部分页点，标题右侧灰色小字显示1/2或2/2，继续自动播放和左右滑动。

- 企业服务九张横向卡片增加克制的淡色渐变、右侧同心弧线及点阵纹理，文案/图标保持清晰，查看全部使用轻量胶囊入口；本轮用户明确要求完成后上线。
